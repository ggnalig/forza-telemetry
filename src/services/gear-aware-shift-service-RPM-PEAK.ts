// Gear-Aware Shift System V3 - RPM PEAK ALTERNATIVE
// Truly gear-ratio-based with RPM peak approach for intuitive driver feedback
...
export class GearAwareShiftService {
  ...
  public calculateShift(telemetry: TelemetryData): ShiftData {
    ...
    // V3: RPM PEAK ALTERNATIVE - Calculate RPM progress and optimal shift point
    ...
    // V3: RPM PEAK ALTERNATIVE - Progressive LED based on RPM progress in gear
  ...
  /**
   * V3: RPM PEAK ALTERNATIVE - Calculate RPM progress within current gear
   * This considers gear characteristics for more intuitive driver feedback
   */
  private calculateRpmProgressInGear(
    ...
    const safeRatio = Math.max(0.1, _currentRatio || 1.0); // Prevent division by zero
    const ratioFactor = Math.min(1.0, safeRatio / 2.0); // Normalize
    const adjustedMaxRpm = practicalMaxRpm * (0.8 + ratioFactor * 0.2);

    // Ensure we don't divide by zero
    const safeAdjustedMaxRpm = Math.max(1000, adjustedMaxRpm);

    return Math.min(1, currentRpm / safeAdjustedMaxRpm);
  }

  /**
   * V3: RPM PEAK ALTERNATIVE - Calculate optimal shift point for this gear
   * Based on gear ratio characteristics (close vs wide ratio)
   */
  ...
  /**
   * V3: RPM PEAK ALTERNATIVE - Generate progressive LED based on RPM progress in gear
   * This is more intuitive for drivers as it matches what they see on the tachometer
   */
  private generateProgressiveLedsV3_RPM_PEAK(
    currentRpm: number,
    maxRpm: number,
    _rpmAfterShift: number,
    state: string,
    blink: boolean,
    _currentGear: number,
    _currentRatio: number,
    ...
    for (let i = 0; i < this.PROGRESSIVE_STEPS; i++) {
      ...
      if (i >= activeLeds) {
        ...
        // OPTIMAL state - show based on RPM progress vs optimal shift point
        if (ledPosition >= optimalShiftPoint) {
          leds.push("🔵"); // Blue: At or past optimal shift point
        } else if (ledPosition >= optimalShiftPoint * 0.7) {
          leds.push("🟡"); // Yellow: Approaching optimal shift point
        } else {
          leds.push("🟢"); // Green: Building up to optimal
        }
      } else if (state === "EARLY") {
        // EARLY state - show based on RPM progress
        if (ledPosition >= 0.6) {
          leds.push("🟡"); // Yellow: Getting higher in RPM
        } else {
          leds.push("🟢"); // Green: Still low RPM
        }
      } else {
    ...
    // Add debug info
    if (leds.length > 0) {
      // This is a bit hacky but works for debug - we'll store it in a way that doesn't affect the array
      // In real implementation, we'd pass this through a different mechanism
      // const debugInfo = {
      //   rpmProgressInGear: Math.round(rpmProgressInGear * 100) / 100,
      //   optimalShiftPoint: optimalShiftPoint,
      //   currentGear: currentGear,
      //   gearSpread: nextRatio / currentRatio,
      // };
      // For now, we'll just return the LEDs and handle debug separately
    }
