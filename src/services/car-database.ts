 ...
 * scripts/build-car-database.ts) since the two games share the same
 * car-ordinal ID space even though their UDP wire formats differ (this
 * backend parses FH6's own 324-byte Data Out format - see src/udp/parser.ts)
 * - returns null for any ordinal not present in either source (a newer DLC
 * car, or one added after the dump was generated) rather than guessing.
 */
